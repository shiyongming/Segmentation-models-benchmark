...
data_root_path="../Downloads/VOCdevkit/VOC2012"
...
def make_one_hot(x,n):
    ...
    for i in range(n):
        ...
        one_hot[x==i][i]=1
    '''
    one_hot = np.zeros([n, x.shape[0], x.shape[1]])
...
class voc_reader:
    ...
    def __init__(self,train_batch_size,val_batch_size):
        ...
        print('Number of training files', self.n_train_file)
        print('Number of validation files', self.n_val_file)
    ...
    def load_file_name_list(self,file_path):
        ...
        with open(file_path, 'r') as file_to_read:
            while True:
                lines = file_to_read.readline().strip()  # ÕûÐÐ¶ÁÈ¡Êý¾Ý
                if not lines:
                    break
                    pass
...
#         val_labels = np.zeros([self.val_batch_size, self.resize_height, self.resize_width, 21])
        ...
        for i in range(self.val_batch_size):
            ...
            #print('index'+str(index))
            img=torchvision.io.read_image(self.row_file_path+self.val_file_name_list[index]+'.jpg')
            img = convert_image_dtype(img, dtype=torch.float)
            normalized_img = F.normalize(img, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))
            #normalized_img_numpy = batch.unsqueeze(0).numpy()
            #img=Image.open(self.row_file_path+self.val_file_name_list[index]+'.jpg')
            #img = np.array(img)
            #val_imgs = np.zeros((self.val_batch_size, 3, img.shape[1], img.shape[2]))
            #print(img.shape)
            val_imgs=normalized_img
            ...
            # print(label.shape)
            one_hot_label = make_one_hot(label, 21)
            val_labels = np.zeros([self.val_batch_size, 21, label.shape[0], label.shape[1]])
            val_labels[i]=one_hot_label
            label_batch = np.zeros([self.val_batch_size, label.shape[0], label.shape[1]])
            label_batch[i] = label
        ...
        return val_imgs, label_batch, val_labels
